...
export const StatWidget = ({ icon: Icon, label, value, color, className = '' }: any) => (
    <div className={`bg-white dark:bg-ph-surface-card border border-gray-100 dark:border-white/5 p-3.5 rounded-2xl flex items-center justify-between shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group ${className}`}>
        <div>
...
export const AssetStatWidget = ({ icon: Icon, label, value, color, onClick }: any) => (
    <button onClick={onClick} className="bg-white dark:bg-ph-surface-card border border-gray-100 dark:border-white/5 p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-gray-200 dark:hover:border-white/10 transition-all duration-300 group h-full w-full">
